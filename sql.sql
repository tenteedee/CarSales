...
CREATE TABLE `feedback`  (
  ...
  INDEX `car_id`(`car_id` ASC) USING BTREE,
  CONSTRAINT `feedback_ibfk_1` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `feedback_ibfk_2` FOREIGN KEY (`car_id`) REFERENCES `cars` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
...
-- ----------------------------
BEGIN;
INSERT INTO `feedback` (`id`, `customer_id`, `car_id`, `rating`, `content`, `created_at`) VALUES (1, 3, 1, 0, 'great car', '2024-11-06 09:37:18'), (2, 16, 3, 5, 'great', '2024-11-07 01:39:42'), (3, 16, 3, 4, 'great', '2024-11-07 01:46:01'), (4, 16, 3, 4, 'great', '2024-11-07 01:57:48'), (5, 16, 3, 4, 'nice', '2024-11-07 02:08:14');
COMMIT;
...
-- ----------------------------
BEGIN;
INSERT INTO `insurance_providers` (`id`, `name`, `phone_number`, `email`, `created_at`, `updated_at`) VALUES (1, 'MIC', '0983673716', 'mic@gmail.com', '2024-10-28 21:31:26', '2024-11-07 01:03:54'), (2, 'Pjico', '0932476234', 'pjico@gmail.com', '2024-10-28 21:32:05', '2024-11-07 01:03:55'), (3, 'Petro', '02345272453', 'petro@gmail.com', '2024-10-28 21:33:26', '2024-11-07 01:03:58'), (4, 'Bảo Việt1', '022748127422', 'baoviet@gmail.com1', '2024-10-28 21:33:57', '2024-11-07 01:04:00');
COMMIT;
...
CREATE TABLE `insurances`  (
  ...
  `price` double NULL DEFAULT NULL COMMENT 'số tiền bh,nếu là 1 -> tính theo % xe,2 là tính giá bth',
  `created_at` timestamp NULL DEFAULT NULL,
...
-- ----------------------------
BEGIN;
INSERT INTO `insurances` (`id`, `name`, `description`, `insurance_provider_id`, `type`, `type_price`, `price`, `created_at`, `updated_at`) VALUES (1, 'Bảo hiểm TNDS', 'Bảo hiểm trách nhiệm dân sự, bắt buộc khi mua xe.', 4, 1, 1, 481000, NULL, NULL), (2, 'Bảo hiểm thân vỏ MIC', 'Bảo hiểm thân vỏ MIC', 1, 2, 2, 1.1, NULL, NULL), (3, 'Bảo hiểm thân vỏ Pjico', 'Bảo hiểm thân vỏ Pjico', 2, 2, 2, 7, NULL, NULL), (4, 'Bảo hiểm thân vỏ PVI', 'Bảo hiểm thân vỏ PVI', 3, 2, 2, 1.15, NULL, '2024-11-07 05:41:10'), (5, 'abcbuff33', 'TungNguyen VPS', 1, 1, 1, 1, '2024-11-07 05:50:04', '2024-11-07 06:26:01');
COMMIT;
...
) ENGINE = InnoDB AUTO_INCREMENT = 17 CHARACTER SET = utf8mb3 COLLATE = utf8mb3_general_ci;

-- ----------------------------
-- Records of order_details
-- ----------------------------
BEGIN;
INSERT INTO `order_details` (`id`, `description`, `order_id`, `price`, `insurance_contract_id`) VALUES (1, 'giá xe', 2, 1.00, NULL), (2, 'chi phí', 2, 11111.00, NULL), (3, 'Giá xe', 162, 500000000.00, NULL), (4, 'Phí kiểm định', 162, 230000.00, NULL), (5, 'Phí đăng ký', 162, 20000000.00, NULL), (6, 'Phí sử dụng đường bộ', 162, 1560000.00, NULL), (7, 'Phí trước bạ', 162, 12.00, NULL), (8, 'Bảo hiểm TNDS', 162, 481000.00, NULL), (9, 'Bảo hiểm thân vỏ', 162, 5500000.00, NULL), (10, 'Giá xe', 163, 300000000.00, NULL), (11, 'Phí kiểm định', 163, 230000.00, NULL), (12, 'Phí đăng ký', 163, 20000000.00, NULL), (13, 'Phí sử dụng đường bộ', 163, 1560000.00, NULL), (14, 'Phí trước bạ', 163, 36000000.00, NULL), (15, 'Bảo hiểm TNDS', 163, 481000.00, NULL), (16, 'Bảo hiểm thân vỏ', 163, 3300000.00, NULL);
COMMIT;
...
CREATE TABLE `orders`  (
  ...
  `customer_id` int NULL DEFAULT NULL,
  `car_id` int NULL DEFAULT NULL,
  `payment_price` decimal(15, 2) NULL DEFAULT NULL,
  ...
  `order_status` enum('pending','confirmed','paying','completed','cancelled') CHARACTER SET utf8mb3 COLLATE utf8mb3_general_ci NULL DEFAULT 'pending',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp,
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp ON UPDATE CURRENT_TIMESTAMP,
...
) ENGINE = InnoDB AUTO_INCREMENT = 164 CHARACTER SET = utf8mb3 COLLATE = utf8mb3_general_ci;

-- ----------------------------
-- Records of orders
-- ----------------------------
BEGIN;
INSERT INTO `orders` (`id`, `customer_id`, `car_id`, `payment_price`, `total_price`, `sales_staff_id`, `technical_staff_id`, `insurance_staff_id`, `order_status`, `created_at`, `updated_at`, `showroom_id`, `loan_id`, `discount_id`) VALUES (1, 3, 1, 1.00, 2.00, 6, 6, 6, 'pending', '2024-11-01 23:28:08', '2024-11-05 10:23:40', 1, NULL, NULL), (2, 12, 7, 1111.00, 1111.00, 8, 11, 10, 'paying', '2024-11-02 00:49:24', '2024-11-05 10:23:46', NULL, NULL, NULL), (161, 21, 1, 582271000.00, 582271000.00, 8, 11, 8, 'pending', '2024-11-07 02:47:53', '2024-11-07 03:23:42', 1, NULL, NULL), (162, 21, 1, 587771000.00, 587771000.00, 8, 1, 17, 'pending', '2024-11-07 03:02:17', '2024-11-07 06:29:19', 1, NULL, NULL), (163, 21, 4, 361571000.00, 361571000.00, 13, 1, 18, 'paying', '2024-11-07 03:11:30', '2024-11-07 06:29:28', 1, NULL, NULL);
COMMIT;
...
) ENGINE = InnoDB AUTO_INCREMENT = 19 CHARACTER SET = utf8mb3 COLLATE = utf8mb3_general_ci;

-- ----------------------------
-- Records of staffs
-- ----------------------------
BEGIN;
INSERT INTO `staffs` (`id`, `fullname`, `password`, `phone_number`, `email`, `role_id`, `created_at`, `updated_at`, `showroom_id`, `address`, `insurance_provider_id`, `avatar_url`) VALUES (1, 'Director TGB', '$2a$12$ZY31bETENlDNwqGmI88olOI1ZFE3DhMWN3pdTWe.YXYVi4ha1CwF2', '0972635782', 'staff1@gmail.com', 1, '2024-09-13 18:34:26', '2024-11-07 04:29:13', 1, 'HN', NULL, 'http://localhost:3001/assets/images/1728274072789-300-1.jpg'), (6, 'Director TTD', '$2b$10$SW72k2ZxcIyJq.g6JqxFAeYKUrGYsTFzbZJOLShqpefdS.cRE8xz2', '0762506738', 'vtd.junoo@gmail.com', 4, '2024-09-30 11:32:41', '2024-10-31 01:37:09', 1, 'dsadsa', NULL, 'http://localhost:3001/assets/images/1729133624952-300-2.jpg'), (8, 'Sale DucTT', '$2a$12$.yb1aDxi5QP/.a2esNs5WuH2sgRmPHaO/q1rZshPcJmewc5JRLjqm', '0762506738', 'duyvthe180740@fpt.edu.vn1', 2, '2024-10-17 00:54:42', '2024-10-31 01:37:29', 1, 'Ha Noi', NULL, 'http://localhost:3001/assets/images/1729734338340-300-2.jpg'), (10, 'Director DuyVT', '$2b$10$ZJDH5J.DXz5DuN2kAkdHPOHgYqvQ9YOJBucQXHigjIrhMjwSsSL3O', '0987654320', 'duyvthe180740@fpt.edu.vna', 4, '2024-10-24 00:18:48', '2024-10-31 01:40:06', 1, 'Ha Noi', NULL, 'http://localhost:3001/assets/images/1730313606189-300-8.jpg'), (11, 'Tech DuyVT', '$2a$12$RApOa4XzIbZ4ph7GYcWYieLFob4D2XzuGkydu4Q.ps.9tBMAWghSm', '0987654321', 'duyvthe180740@fpt.edu.vn', 1, '2024-10-24 08:42:52', '2024-10-31 01:37:55', 4, '94-96 Phạm Đình Hổ, P2, Q6, HCM1', NULL, NULL), (12, 'Director AnhNT', '$2b$10$0M7ni1tYtsMlmhODrp0eSuhr/kqnBo78HcAC9zce2XlGE08vwToem', '0987654322', 'anhnthe180055@fpt.edu.vn', 4, '2024-10-29 10:04:18', '2024-11-06 19:11:09', 1, 'sdadsa', NULL, NULL), (13, 'Sale DuyVT', '$2b$10$R4W.lVgu4q61E8VxYXQqKOgwx9rOe7ItdYmJrm.S67L2pd5VyIX0G', '0987654432', 'saleduyvt@gmail.com', 2, '2024-10-31 08:52:45', '2024-10-31 08:52:46', 1, 'Hà Nội', NULL, NULL), (14, 'Sale DucTT2', '$2b$10$wwj8GKxOFCY/YJNze6v3HewAyAel.dugQbHPSL.Ywy2On1tSB.F0K', '0987654432', 'tranduc2004nd01@gmail.com', 2, '2024-10-31 09:16:50', '2024-10-31 09:16:50', 1, 'Hà Nội', NULL, NULL), (16, 'thuc anh', '$2y$10$r02k4Eh9Ce.wl3D6mbERweSbKdcmHu0Uf0m4GoGjtY5xzHzU/R4eK', '039517457514', 'nguyenthucanh0004@gmail.com', 4, '2024-11-05 10:25:27', '2024-11-05 10:25:38', 1, 'hanoi', NULL, NULL), (17, 'Tran Tien Duc', '$2b$10$LY9dWmhWjGVwDFt6Vp6mUOJsdyf3i1ZJ8d6mxkFipcjOyaV9xwzv.', '0987654432', 'tranduc248nd@gmail.com', 3, '2024-11-05 11:55:10', '2024-11-07 04:09:37', 1, 'Hà Nội', NULL, NULL), (18, 'Insurance Duy', '$2a$12$ZY31bETENlDNwqGmI88olOI1ZFE3DhMWN3pdTWe.YXYVi4ha1CwF2', '1', '1@gmail.com', 3, '2024-11-07 04:10:18', '2024-11-07 04:10:30', 1, NULL, NULL, NULL);
COMMIT;
...
) ENGINE = InnoDB AUTO_INCREMENT = 45 CHARACTER SET = utf8mb3 COLLATE = utf8mb3_general_ci;

-- ----------------------------
-- Records of test_drive_requests
-- ----------------------------
BEGIN;
INSERT INTO `test_drive_requests` (`id`, `customer_id`, `car_id`, `test_drive_date`, `sales_staff_id`, `status`, `created_at`, `updated_at`, `showroom_id`) VALUES (21, 11, 3, '2024-10-31 07:48:54', 8, 'completed', '2024-10-31 07:31:35', '2024-10-31 07:48:54', 1), (28, 11, 5, '2024-10-31 09:22:57', 14, 'approved', '2024-10-31 09:18:11', '2024-10-31 09:22:57', 1), (29, 11, 11, '2024-11-02 00:50:53', 14, 'completed', '2024-11-02 00:45:51', '2024-11-02 00:50:54', 1), (35, 16, 8, '2024-11-05 00:33:33', 8, 'approved', '2024-11-05 00:14:14', '2024-11-05 00:33:33', 1), (36, 20, 3, '2024-11-05 00:41:29', 13, 'cancelled', '2024-11-05 00:36:30', '2024-11-05 01:02:48', 1), (37, 20, 7, '2024-11-05 10:04:25', 8, 'approved', '2024-11-05 01:30:28', '2024-11-05 10:04:25', 1), (38, 20, 3, '2024-11-05 10:21:01', 13, 'approved', '2024-11-05 10:20:41', '2024-11-05 10:21:01', 1), (39, 20, 1, '2024-11-14 00:00:00', 13, 'pending', '2024-11-05 10:33:44', '2024-11-05 10:33:44', 1), (40, 11, 3, '2024-11-15 00:00:00', 13, 'pending', '2024-11-05 10:57:40', '2024-11-05 10:57:40', 1), (41, 16, 7, '2024-11-05 11:33:42', 13, 'cancelled', '2024-11-05 11:26:49', '2024-11-05 11:33:41', 1), (42, 16, 3, '2024-11-05 11:35:54', 14, 'completed', '2024-11-05 11:34:49', '2024-11-05 11:35:53', 1), (43, 22, 1, '2024-11-05 11:56:24', 14, 'approved', '2024-11-05 11:38:55', '2024-11-05 11:56:24', 1), (44, 20, 3, '2024-11-05 19:38:29', 14, 'approved', '2024-11-05 19:37:50', '2024-11-05 19:38:29', 1);
COMMIT;
